...
import SalePlanSwitchDefault from '../../../src/functions/sale-plan/SalePlanSwitchDefault';

const salePlanSwitchDefault = new SalePlanSwitchDefault();

describe('SalePlanSwitchDefault', () => {
    it('should pass if request with sufficient field when checking pre-requirement', async () => {
        ...
        const res = await salePlanSwitchDefault.preRequirement(req as any);
    ...
    it('should pass if request with admin auth when check permission', async () => {
        ...
        const permission = await salePlanSwitchDefault.permission(req as any);
    ...
    it('should response 204 if switch success', async () => {
        ...
        res = await salePlanSwitchDefault.function(req as any, res as any) as any;

        expect(res.statusCode).to.be.equal(204);

        expect(MockFetch.requests[0].reqUrl).to.be.equal('http://127.0.0.1:9999/salePlan/fda2c091-3e6a-4a25-9060-c8e0ad741655/switchDefault');
    });
