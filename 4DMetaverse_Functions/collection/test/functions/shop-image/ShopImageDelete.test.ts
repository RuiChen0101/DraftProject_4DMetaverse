...
import ShopImageDelete from '../../../src/functions/shop-image/ShopImageDelete';

const shopImageDelete = new ShopImageDelete();

describe('ShopImageDelete', () => {
    it('should pass if request with sufficient field when checking pre-requirement', async () => {
        ...
        const res = await shopImageDelete.preRequirement(req as any);
    ...
    it('should pass if request with admin auth when check permission', async () => {
        ...
        const permission = await shopImageDelete.permission(req as any);
    ...
    it('should response 204 if delete success', async () => {
        ...
        res = await shopImageDelete.function(req as any, res as any) as any;

        expect(res.statusCode).to.be.equal(204);

        expect(MockFetch.requests[0].reqUrl).to.be.equal('http://127.0.0.1:9999/shopImage/1/delete');
    });
